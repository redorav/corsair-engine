...
namespace crgfx
{
	CommandBufferD3D12::CommandBufferD3D12(crgfx::IDevice* renderDevice, const crgfx::CommandBufferDescriptor& descriptor)
		: crgfx::ICommandBuffer(renderDevice, descriptor)
	...
	void CommandBufferD3D12::WriteTextureSRV(const TextureBinding& textureBinding, D3D12_CPU_DESCRIPTOR_HANDLE& srvHandle)
	{
	...
	void CommandBufferD3D12::WriteSamplerView(const crgfx::ISampler* sampler, D3D12_CPU_DESCRIPTOR_HANDLE& samplerHandle)
	{
	...
	void CommandBufferD3D12::WriteRWTextureUAV(const RWTextureBinding& rwTextureBinding, D3D12_CPU_DESCRIPTOR_HANDLE& uavHandle)
	{
	...
	void CommandBufferD3D12::BeginPS()
	{
	...
	void CommandBufferD3D12::ClearRenderTargetPS(const crgfx::ITexture* renderTarget, const float4& color, uint32_t mip, uint32_t slice, uint32_t mipCount, uint32_t sliceCount)
	{
	...
	void CommandBufferD3D12::BeginDebugEventPS(const char* eventName, const float4& color)
	{
	...
	void CommandBufferD3D12::EndDebugEventPS()
	{
		PIXEndEvent(m_d3d12GraphicsCommandList);
	}

	void CommandBufferD3D12::InsertDebugMarkerPS(const char* markerName, const float4& color)
	{
		float4 color255 = color * 255.0f;
		BYTE r = (BYTE)(color255.r);
		BYTE g = (BYTE)(color255.g);
