...
namespace crgfx
{
	DeviceD3D12::DeviceD3D12(IGraphicsSystem* renderSystem, const DeviceDescriptor& descriptor) : IDevice(renderSystem, descriptor)
	{
		...
		m_deviceProperties.vendor = crgfx::GraphicsVendor::FromVendorID(adapterDescriptor.VendorId);
		m_deviceProperties.graphicsApiDisplay.append_sprintf("%s %s", crgfx::GraphicsApi::ToString(m_deviceProperties.graphicsApi), GetD3DFeatureLevelString(featureLevels.MaxSupportedFeatureLevel));

#if defined(USE_AGILITY_SDK)
		if (usingAgility)
		{
			m_deviceProperties.graphicsApiDisplay.append_sprintf(", Agility SDK %i", D3D12SDKVersion);
		}
#endif

		m_deviceProperties.description.append_convert<wchar_t>(adapterDescriptor.Description);
		m_deviceProperties.gpuMemoryBytes = adapterDescriptor.DedicatedVideoMemory;
		...
		{
			m_deviceProperties.isUMA = d3d12Architecture.UMA;
		}

		D3D12_FEATURE_DATA_D3D12_OPTIONS d3d12Options = {};
		if (m_d3d12Device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &d3d12Options, sizeof(d3d12Options)) == S_OK)
		{
			m_deviceProperties.features.conservativeRasterization = d3d12Options.ConservativeRasterizationTier >= D3D12_CONSERVATIVE_RASTERIZATION_TIER_1;
		}
		...
		{
			m_deviceProperties.features.raytracing = d3d12Options5.RaytracingTier >= D3D12_RAYTRACING_TIER_1_0;
		}
		...
		if (m_d3d12Device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS12, &d3d12Options12, sizeof(d3d12Options12)) == S_OK)
		{
			...
			m_deviceProperties.features.textureFormatCasting = d3d12Options12.RelaxedFormatCastingSupported;
		}
