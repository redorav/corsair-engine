#include "Graphics/CrRendering_pch.h"

#include "GraphicsSystemD3D12.h"
#include "DeviceD3D12.h"

#include "Graphics/Extensions/CrNVAPIHeader.h"

#include "Core/CrMacros.h"
#include "Core/Logging/ICrDebug.h"

namespace crgfx
{
	GraphicsSystemD3D12::GraphicsSystemD3D12(const crgfx::GraphicsSystemDescriptor& graphicsSystemDescriptor)
		: IGraphicsSystem(graphicsSystemDescriptor)
		, m_d3d12DebugController(nullptr)
		, m_d3d12DebugController1(nullptr)
	{
		// Load NVAPI or other extension mechanisms before we load Renderdoc or PIX
		if (graphicsSystemDescriptor.enableNVAPI)
		{
			InitializeNVAPI();
		}

		if (graphicsSystemDescriptor.enableRenderDoc)
		{
			InitializeRenderdoc();
		}
		else if (graphicsSystemDescriptor.enablePIX)
		{
			/*m_pixEnabled = */InitializePIX();
		}

		UINT createFactoryFlags = 0;
		HRESULT hResult = S_OK;

		if (graphicsSystemDescriptor.enableValidation)
		{
			createFactoryFlags |= DXGI_CREATE_FACTORY_DEBUG;

			// Enable standard validation layers
			hResult = D3D12GetDebugInterface(IID_PPV_ARGS(&m_d3d12DebugController));
			if (SUCCEEDED(hResult))
			{
				m_d3d12DebugController->EnableDebugLayer();

				hResult = m_d3d12DebugController->QueryInterface(IID_PPV_ARGS(&m_d3d12DebugController1));

				// Enable GPU based validation
				if (SUCCEEDED(hResult))
				{
					m_d3d12DebugController1->SetEnableGPUBasedValidation(true);
					m_d3d12DebugController1->SetEnableSynchronizedCommandQueueValidation(true);
				}
			}
		}

		// Assume DXGI 1.4
		hResult = CreateDXGIFactory2(createFactoryFlags, IID_PPV_ARGS(&m_dxgiFactory4));
		CrAssertMsg(SUCCEEDED(hResult), "Failed to create DXGIFactory");

		if (m_dxgiFactory4->QueryInterface(IID_PPV_ARGS(&m_dxgiFactory6)) == S_OK)
		{

		}

		if (m_dxgiFactory4->QueryInterface(IID_PPV_ARGS(&m_dxgiFactory7)) == S_OK)
		{

		}
	}

	crgfx::IDevice* GraphicsSystemD3D12::CreateDevicePS(const crgfx::DeviceDescriptor& descriptor)
	{
		return new DeviceD3D12(this, descriptor);
	}

	bool GraphicsSystemD3D12::InitializeNVAPI()
	{
#if defined(CR_NVAPI_SUPPORTED)

		if (!m_nvapiInitialized)
		{
			NvAPI_Status result = NvAPI_Initialize();
			m_nvapiInitialized = (result == NVAPI_OK);
		}

#endif

		return m_nvapiInitialized;
	}

	bool GraphicsSystemD3D12::InitializePIX()
	{
		if (!m_pixInitialized)
		{
			m_pixInitialized = m_pix.Initialize();
		}

		return m_pixInitialized;
	}
};