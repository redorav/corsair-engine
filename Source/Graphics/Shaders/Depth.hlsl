...
RWTexture2D<float2> RWLinearDepthMinMaxMip5;
...
// to do the rest
void DepthDownsampleLinearizeMinMax(uint2 dispatchThreadID, uint2 groupThreadID)
{
	uint2 depthTextureResolution;
	RawDepthTexture.GetDimensions(depthTextureResolution.x, depthTextureResolution.y);

	uint2 pixelCoordMip1 = dispatchThreadID;
	...
	// We linearize for the first pass, then no need to linearize further after that
	...
	// Process Mip 5
	if ((pixelCoordMip1.x % 16) == 0 && (pixelCoordMip1.y % 16) == 0)
	{
		float2 minMaxTL = gs_minMaxDepth[groupThreadID.x + 0][groupThreadID.y + 0];
		float2 minMaxTR = gs_minMaxDepth[groupThreadID.x + 15][groupThreadID.y + 0];
		float2 minMaxBL = gs_minMaxDepth[groupThreadID.x + 0][groupThreadID.y + 15];
		float2 minMaxBR = gs_minMaxDepth[groupThreadID.x + 15][groupThreadID.y + 15];

		float minDepthMip4 = min4(minMaxTL.x, minMaxTR.x, minMaxBL.x, minMaxBR.x);
		float maxDepthMip4 = max4(minMaxTL.y, minMaxTR.y, minMaxBL.y, minMaxBR.y);

		RWLinearDepthMinMaxMip5[pixelCoordMip1 / 16] = float2(minDepthMip4, maxDepthMip4);
	}
}

[numthreads(DEPTH_DOWNSAMPLE_GROUP_SIZE, DEPTH_DOWNSAMPLE_GROUP_SIZE, 1)]
void DepthDownsampleLinearizeMinMaxCS(uint2 dispatchThreadID : SV_DispatchThreadID, uint2 groupThreadID : SV_GroupThreadID)
{
	DepthDownsampleLinearizeMinMax(dispatchThreadID, groupThreadID);
}
