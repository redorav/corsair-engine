...
namespace crgfx
{
	DeviceVulkan::DeviceVulkan(IGraphicsSystem* renderSystem, const crgfx::DeviceDescriptor& descriptor) : IDevice(renderSystem, descriptor)
		, m_numCommandQueues(0)
	{
		m_vkInstance = static_cast<const GraphicsSystemVulkan*>(renderSystem)->GetVkInstance();
	...
	VkResult DeviceVulkan::SelectPhysicalDevice()
	{
		VkResult result;
		...
		crgfx::GraphicsVendor::T preferredVendor = m_deviceProperties.preferredVendor;
		...
		for (uint32_t i = 0; i < m_vkPhysicalDeviceMemoryProperties.memoryHeapCount; ++i)
		{
			...
			{
				m_deviceProperties.gpuMemoryBytes += memoryHeap.size;
			}
		}
		...
		m_deviceProperties.graphicsApiDisplay.append_sprintf("%s %d.%d.%d", crgfx::GraphicsApi::ToString(crgfx::GraphicsApi::Vulkan), vulkanVersionMajor, vulkanVersionMinor, vulkanVersionPatch);

		// Populate the render device properties into the platform-independent structure
		m_deviceProperties.vendor = crgfx::GraphicsVendor::FromVendorID(m_vkPhysicalDeviceProperties2.properties.vendorID);
		...
		if (m_deviceProperties.vendor == crgfx::GraphicsVendor::NVIDIA)
		{
			driverId = VK_DRIVER_ID_NVIDIA_PROPRIETARY;
		}
		else if (m_deviceProperties.vendor == crgfx::GraphicsVendor::Intel)
		{
			driverId = VK_DRIVER_ID_INTEL_PROPRIETARY_WINDOWS;
		}
		else if (m_deviceProperties.vendor == crgfx::GraphicsVendor::AMD)
		{
		...
		m_deviceProperties.driverVersion.major = VK_API_VERSION_MAJOR(driverVersionEncoded);
		m_deviceProperties.driverVersion.minor = VK_API_VERSION_MINOR(driverVersionEncoded);
		m_deviceProperties.driverVersion.patch = VK_API_VERSION_PATCH(driverVersionEncoded);
		...
		{
			m_deviceProperties.driverVersion.major = ((uint32_t)(driverVersionEncoded) >> (8 + 8 + 6)) & 0x3ff;
			m_deviceProperties.driverVersion.minor = ((uint32_t)(driverVersionEncoded) >> (8 + 6)) & 0xff;

			uint32_t secondary = ((uint32_t)(driverVersionEncoded) >> 6) & 0x0ff;
			uint32_t tertiary = driverVersionEncoded & 0x03f;

			m_deviceProperties.driverVersion.patch = (secondary << 8) | tertiary;
		}
		else if (driverId == VK_DRIVER_ID_INTEL_PROPRIETARY_WINDOWS)
		{
			m_deviceProperties.driverVersion.major = ((uint32_t)(driverVersionEncoded) >> 14) & 0x3fff;
			m_deviceProperties.driverVersion.minor = (uint32_t)(driverVersionEncoded) & 0x3fff;
			m_deviceProperties.driverVersion.patch = 0;
		}

		m_deviceProperties.description = m_vkPhysicalDeviceProperties2.properties.deviceName;
		m_deviceProperties.maxConstantBufferRange = m_vkPhysicalDeviceProperties2.properties.limits.maxUniformBufferRange;
		m_deviceProperties.maxTextureDimension1D = m_vkPhysicalDeviceProperties2.properties.limits.maxImageDimension1D;
		m_deviceProperties.maxTextureDimension2D = m_vkPhysicalDeviceProperties2.properties.limits.maxImageDimension2D;
		m_deviceProperties.maxTextureDimension3D = m_vkPhysicalDeviceProperties2.properties.limits.maxImageDimension3D;
		m_deviceProperties.isUMA = m_vkPhysicalDeviceProperties2.properties.deviceType & VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU;
	...
	ICrSampler* DeviceVulkan::CreateSamplerPS(const CrSamplerDescriptor& descriptor)
	{
	...
	VkResult DeviceVulkan::CreateLogicalDevice()
	{
		...
		if (IsVkDeviceExtensionSupported(VK_EXT_CONSERVATIVE_RASTERIZATION_EXTENSION_NAME))
		{
			...
			m_deviceProperties.features.conservativeRasterization = true;
		}
		...
		m_deviceProperties.features.tessellation = m_vkDeviceSupportedFeatures2.features.tessellationShader;
		m_deviceProperties.features.geometryShaders = m_vkDeviceSupportedFeatures2.features.geometryShader;
		m_deviceProperties.features.compressionBC = m_vkDeviceSupportedFeatures2.features.textureCompressionBC;
		m_deviceProperties.features.compressionETC = m_vkDeviceSupportedFeatures2.features.textureCompressionETC2;
		m_deviceProperties.features.compressionASTC = m_vkDeviceSupportedFeatures2.features.textureCompressionASTC_LDR;

		return vkResult;
	}

	bool DeviceVulkan::IsVkDeviceExtensionSupported(const crstl::string& extension)
	{
		return m_supportedDeviceExtensions.find(extension) != m_supportedDeviceExtensions.end();
	}

	uint32_t DeviceVulkan::GetVkMemoryType(uint32_t typeBits, VkFlags properties) const
	{
	...
	uint32_t DeviceVulkan::ReserveVkQueueIndex()
	{
	...
	uint32_t DeviceVulkan::GetVkQueueMaxCount() const
	{
		// TODO There is a maximum based on type of command queue and capability
		return m_maxCommandQueues;
	}

	uint32_t DeviceVulkan::GetVkQueueFamilyIndex() const
	{
		// TODO Need to return an index based on type of queue, capabilities, etc.
		return m_commandQueueFamilyIndex;
	}

	VkQueue DeviceVulkan::GetVkGraphicsQueue() const
	{
		return m_vkGraphicsQueue;
	}

	VkCommandBuffer DeviceVulkan::GetVkSwapchainCommandBuffer() const
	{
		return m_vkSwapchainCommandBuffer;
	}

	VkCommandPool DeviceVulkan::GetVkGraphicsCommandPool() const
	{
	...
	void DeviceVulkan::TransitionVkTextureToInitialLayout(const CrTextureVulkan* vulkanTexture, const crgfx::TextureState& textureState)
	{
		...
		const crvk::VkImageTransitionInfo& imageStateInfo = crvk::GetVkImageStateInfo(vulkanTexture->GetFormat(), textureState.layout);
	...
	void DeviceVulkan::SetVkObjectName(uint64_t vkObject, VkObjectType objectType, const char* name) const
	{
