...
namespace crgfx
{
	GraphicsSystemVulkan::GraphicsSystemVulkan(const crgfx::GraphicsSystemDescriptor& graphicsSystemDescriptor) : IGraphicsSystem(graphicsSystemDescriptor)
	{
	...
	bool GraphicsSystemVulkan::IsVkInstanceExtensionSupported(const crstl::string& extension)
	{
		return m_supportedInstanceExtensions.count(extension) > 0;
	}

	bool GraphicsSystemVulkan::IsVkInstanceLayerSupported(const crstl::string& layer)
	{
		return m_supportedInstanceLayers.count(layer) > 0;
	}

	crgfx::IDevice* GraphicsSystemVulkan::CreateDevicePS(const crgfx::DeviceDescriptor& descriptor)
	{
		return new DeviceVulkan(this, descriptor);
	}
};